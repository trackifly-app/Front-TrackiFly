...
const OrderView = () => {
  ...
  return (
    <Formik
      ...
      validate={(values) => {
        const customerType: 'user' | 'company' = userData?.user?.role?.name === 'company' || userData?.user?.role?.name === 'operator' ? 'company' : 'user';
      ...
      onSubmit={async (values, { setSubmitting }) => {
        if (distance === 0) {
          ...
          showToast('Selecciona origen y destino válidos en el mapa para calcular la distancia.', 'warning');
          setSubmitting(false);
          return;
        }
        if (!userData?.user?.id) {
        ...
        try {
          ...
          showToast('¡Pedido creado! Completá el pago en la ventana que se abrió.', 'success');

          setTimeout(() => {
            router.push(dashboardPath);
          }, 3000);
        } catch (error: any) {
          showToast(error.message || 'Hubo un problema al procesar el pago. Reintentá.', 'error');
        } finally {
          setSubmitting(false);
        }
      }}
                    ...
                    <div className="space-y-4">
                      ...
                      {(userData?.user?.role?.name === 'company' || userData?.user?.role?.name === 'operator') && (
                        <div className="bg-primary/10 border-l-4 border-primary p-4 mb-4 rounded-r-md">
                          <p className="text-sm font-bold text-primary uppercase tracking-wider">Beneficio Exclusivo: Empresa</p>
                          <p className="text-xs text-muted-foreground">Se esta aplicando un 20% de descuento automático a tu tarifa solo por ser parte de nuestra comunidad de empresas.</p>
                        </div>
                      )}
