    ...
    <Formik
      ...
      validate={(values) => {
        ...
        if (!values.delivery_direction)
          errors.delivery_direction = "Destino requerido";
                    ...
                    <div className="grid gap-4">
                      <div>
                        <Field
                          ...
                          className={`${inputStyle} ${errors.name && (touched.name || submitCount > 0) ? "border-red-500" : ""}`}
                        />
                      ...
                      <div>
                        <Field
                          ...
                          className={`${inputStyle} h-24 resize-none ${errors.description && (touched.description || submitCount > 0) ? "border-red-500" : ""}`}
                        />
                        {errors.description &&
                          (touched.description || submitCount > 0) && (
                            <p className={errorLabel}>{errors.description}</p>
                          )}
                      </div>
                        ...
                        <div>
                          ...
                          {errors.category_id &&
                            (touched.category_id || submitCount > 0) && (
                              <p className={errorLabel}>{errors.category_id}</p>
                            )}
                        </div>
                        <div>
                          <Field
                            ...
                            className={`${inputStyle} ${errors.image && (touched.image || submitCount > 0) ? "border-red-500" : ""}`}
                          />
                          {errors.image &&
                            (touched.image || submitCount > 0) && (
                              <p className={errorLabel}>{errors.image}</p>
                            )}
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 items-start">
                        <div className="flex-1">
                          <Field
                            ...
                            placeholder="Altura"
                            type="number"
                            className={`${inputStyle} ${errors.height && (touched.height || submitCount > 0) ? "border-red-500" : ""}`}
                          />
                          {errors.height &&
                            (touched.height || submitCount > 0) && (
                              <p className={errorLabel}>{errors.height}</p>
                            )}
                        </div>
                        <div className="flex-1">
                          <Field
                            ...
                            placeholder="Ancho"
                            type="number"
                            className={`${inputStyle} ${errors.width && (touched.width || submitCount > 0) ? "border-red-500" : ""}`}
                          />
                          {errors.width &&
                            (touched.width || submitCount > 0) && (
                              <p className={errorLabel}>{errors.width}</p>
                            )}
                        </div>
                        <div className="flex-1">
                          <Field
                            ...
                            type="number"
                            className={`${inputStyle} ${errors.depth && (touched.depth || submitCount > 0) ? "border-red-500" : ""}`}
                          />
                          {errors.depth &&
                            (touched.depth || submitCount > 0) && (
                              <p className={errorLabel}>{errors.depth}</p>
                            )}
                        </div>
                        <Field
                          as="select"
                          name="unit"
                          className="h-12.5 rounded-xl border border-border bg-surface-muted px-2 font-bold"
                        >
                      ...
                      <div>
                        <Field
                          ...
                          placeholder="Peso (kg)"
                          type="number"
                          className={`${inputStyle} ${errors.weight && (touched.weight || submitCount > 0) ? "border-red-500" : ""}`}
                        />
                        {errors.weight &&
                          (touched.weight || submitCount > 0) && (
                            <p className={errorLabel}>{errors.weight}</p>
                          )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl bg-surface p-8 shadow-xl text-foreground border border-border h-fit">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="pt-4 space-y-3">
                        {["fragile", "dangerous", "cooled", "urgent"].map(
                          (serv) => (
                            <label
                              key={serv}
                              className="flex items-center gap-3 cursor-pointer hover:text-primary text-xs font-bold uppercase"
                            >
                              <Field
                                type="checkbox"
                                name={serv}
                    ...
                    <button
                      ...
                      className="mt-8 w-full rounded-xl bg-primary py-4 font-black text-white hover:bg-primary-hover transition-all active:scale-95 shadow-lg uppercase"
                    >
                      {isSubmitting ? "REGISTRANDO..." : "CONFIRMAR ENVÍO"}
                    </button>
                    {Object.keys(errors).length > 0 && submitCount > 0 && (
                      <p className="text-red-500 text-[10px] mt-4 text-center font-bold animate-pulse uppercase">
                        Corrija los errores resaltados
                      </p>
                    )}
                  </div>
                </div>
