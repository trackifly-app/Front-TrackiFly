...
const OrderView = () => {
  ...
  if (!L) return <div className="p-10 text-center font-bold text-[#D96B4A]">Iniciando Trackifly...</div>;
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="grid gap-4">
                      <div>
                        <Field name="name" placeholder="Nombre" className={inputStyle} />
                        {errors.name && touched.name && <p className="text-red-500 text-[10px] mt-1 text-left">{errors.name}</p>}
                      </div>
                      
                      <Field as="textarea" name="description" placeholder="Descripción..." className={`${inputStyle} h-20`} />
                      
                      <div className="grid gap-4 md:grid-cols-2">
                        <div>
                          <Field as="select" name="category_id" className={inputStyle}>
                            <option value="">Selecciona Categoría...</option>
                            {CATEGORIES.map((c) => (<option key={c.id} value={c.id}>{c.name}</option>))}
                          </Field>
                          {errors.category_id && touched.category_id && <p className="text-red-500 text-[10px] mt-1 text-left">{errors.category_id}</p>}
                        </div>
                        <div>
                          <Field name="image" placeholder="Imagen URL" className={inputStyle} />
                          {errors.image && touched.image && <p className="text-red-500 text-[10px] mt-1 text-left">{errors.image}</p>}
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1">
                          ...
                          {errors.height && touched.height && <p className="text-red-500 text-[10px] mt-1">{errors.height}</p>}
                        </div>
                        <div className="flex-1">
                          <Field name="width" placeholder="Ancho" type="number" className={inputStyle} />
                          {errors.width && touched.width && <p className="text-red-500 text-[10px] mt-1">{errors.width}</p>}
                        </div>
                        <div className="flex-1">
                          <Field name="depth" placeholder="Profundidad" type="number" className={inputStyle} />
                          {errors.depth && touched.depth && <p className="text-red-500 text-[10px] mt-1">{errors.depth}</p>}
                        </div>
                        <Field as="select" name="unit" className="rounded-xl border border-gray-300 bg-gray-50 px-3 font-bold text-gray-700 outline-none h-[50px]">
                      ...
                      <div>
                        <Field name="weight" placeholder="Peso (kg)" type="number" className={inputStyle} />
                        {errors.weight && touched.weight && <p className="text-red-500 text-[10px] mt-1 text-left">{errors.weight}</p>}
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 mb-4">
                      <div>
                        <Field
                          ...
                          onBlur={(e: any) => handleSearch(e.target.value, "origen", setFieldValue, setFieldTouched)}
                        />
                        {errors.pickup_direction && touched.pickup_direction && <p className="text-red-500 text-[10px] mt-1 text-left">{errors.pickup_direction}</p>}
                      </div>
                      <div>
                        <Field
                          ...
                          placeholder="Destino"
                          className={inputStyle}
                          onBlur={(e: any) => handleSearch(e.target.value, "destino", setFieldValue, setFieldTouched)}
                        />
                        {errors.delivery_direction && touched.delivery_direction && <p className="text-red-500 text-[10px] mt-1 text-left">{errors.delivery_direction}</p>}
                      </div>
                    </div>
                ...
                <aside className="h-fit rounded-2xl bg-[#1a232e] p-8 shadow-xl text-white sticky top-10 text-left">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between border-b border-gray-700 pb-2 text-sm text-gray-400">
                      <span>Trayecto</span><span>{distance.toFixed(1)} km</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-700 pb-2 text-sm text-gray-400">
                    ...
                    <div className="flex justify-between border-b border-gray-700 pb-2 text-sm text-gray-400">
                      <span>Peso</span>
                      <span>
                        {pesoNum} kg {recargoPeso > 0 && <span className="text-[#D96B4A] ml-1"> (+{(recargoPeso * 100).toFixed(0)}%)</span>}
                      </span>
                    </div>

                    <div className="pt-4 space-y-3">
                      <label className="flex items-center gap-2 cursor-pointer hover:text-[#D96B4A] transition-colors"><Field type="checkbox" name="fragile" className="accent-[#D96B4A]" /> Frágil (+15%)</label>
                      <label className="flex items-center gap-2 cursor-pointer hover:text-[#D96B4A] transition-colors"><Field type="checkbox" name="dangerous" className="accent-[#D96B4A]" /> Peligroso (+30%)</label>
                      <label className="flex items-center gap-2 cursor-pointer hover:text-[#D96B4A] transition-colors"><Field type="checkbox" name="cooled" className="accent-[#D96B4A]" /> Refrigerado (+20%)</label>
                      <label className="flex items-center gap-2 cursor-pointer hover:text-[#D96B4A] transition-colors"><Field type="checkbox" name="urgent" className="accent-[#D96B4A]" /> Urgente (+50%)</label>
                    </div>
                  ...
                  <button
                    ...
                    className="mt-8 w-full rounded-xl bg-[#D96B4A] py-4 font-bold text-white hover:bg-[#bf5a3a] transition-all active:scale-95 disabled:opacity-50"
                  >
