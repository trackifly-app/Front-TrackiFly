...
const RegisterView = () => {
  ...
  const inputStyle = 'mt-1 w-full rounded-xl border border-border bg-surface-muted py-2.5 px-4 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40 transition-colors placeholder:text-muted';
        ...
        <div className="w-full max-w-2xl min-h-195 bg-surface rounded-2xl shadow-md p-8 sm:p-10 border border-border transition-colors duration-300 flex flex-col justify-center">
          ...
          <div className="mt-2 text-center text-sm text-muted space-y-1">
            ...
            <p>
              ...
              <Link href="/registercompany" className="text-primary font-medium hover:underline">
                Registrá tu Empresa
              </Link>
            </p>
          </div>
              ...
              <Form className="mt-8 space-y-5 text-left">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                  ...
                  <div>
                    ...
                    <Field type="date" name="birthdate" max={new Date().toISOString().split('T')[0]} className={inputStyle} />
                    <ErrorMessage name="birthdate" component="div" className="text-xs text-primary mt-1" />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative mt-1">
                      ...
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-muted hover:text-foreground transition-colors">
                  ...
                  <div>
                    <label className="text-sm font-medium text-muted">Género</label>
                    ...
                    <ErrorMessage name="gender" component="div" className="text-xs text-primary mt-1" />
                  </div>
                ...
                <button type="submit" disabled={!isValid || isSubmitting} className="w-full rounded-xl bg-primary py-3.5 text-sm font-bold text-white transition hover:bg-primary-hover disabled:bg-surface-muted disabled:text-muted mt-4 flex justify-center items-center">
      ...
      <div className="w-full lg:w-1/2 flex flex-col bg-[#1f2a37] dark:bg-slate-900 p-10 lg:p-16 text-white transition-colors duration-300">
        <div className="flex flex-col flex-1 justify-center">
          <div className="mb-10 text-left">
            ...
            <p className="mt-4 text-gray-300 max-w-md text-lg">Unite a TrackiFly y gestioná tus envíos personales de forma simple, rápida y segura.</p>
          </div>
          <ul className="space-y-4 text-lg text-gray-300 text-left">
            <li className="flex items-center gap-3">
              <span className="text-primary text-xl">✔</span> Registro rápido y sin complicaciones
            </li>
            <li className="flex items-center gap-3">
              <span className="text-primary text-xl">✔</span> Acceso inmediato a tu panel de envíos
            </li>
            <li className="flex items-center gap-3">
              <span className="text-primary text-xl">✔</span> Seguimiento en tiempo real de tus paquetes
            </li>
          </ul>
        </div>
