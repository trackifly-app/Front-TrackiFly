    ...
    <nav className="w-full border-b border-gray-200 dark:border-zinc-800 sticky top-0 bg-white dark:bg-zinc-950 z-50">
      ...
      <div className="w-full grid grid-cols-[auto_1fr_auto] items-center px-6 py-3">
        ...
        <div className="hidden md:flex items-center gap-6 justify-center list-none">
          <Link href="/" className="text-sm text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white transition-colors">
            Home
          </Link>
          <Link href="/orders" className="text-sm text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white transition-colors">
            Pedidos
          </Link>
          <Link href="/about" className="text-sm text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white transition-colors">
            Conocenos
          </Link>
        </div>
        ...
        <div className="md:hidden flex justify-end">
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-primary dark:text-white p-2 transition-transform active:scale-90" aria-label={isMobileMenuOpen ? 'Cerrar menú' : 'Abrir menú'}>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <Link href="/login" className="text-sm px-5 py-2 rounded-full border border-primary text-primary hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors">
            Ingresar
          </Link>
          <Link href="/register" className="text-sm px-5 py-2 rounded-full bg-primary text-white hover:bg-primary-hover transition-colors shadow-sm">
            Registrarse
          </Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden flex flex-col bg-white dark:bg-zinc-950 px-6 py-4 gap-4 border-t dark:border-zinc-800 animate-in slide-in-from-top duration-300">
          ...
          <div className="flex flex-col gap-3">
            <Link href="/" className="dark:text-white py-1 font-medium" onClick={closeMobileMenu}>
              Home
            </Link>
            <Link href="/orders" className="dark:text-white py-1 font-medium" onClick={closeMobileMenu}>
              Pedidos
            </Link>
            <Link href="/about" className="dark:text-white py-1 font-medium" onClick={closeMobileMenu}>
              Conocenos
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-3 mt-2">
            ...
            <Link href="/register" className="text-center py-2.5 bg-primary text-white rounded-full font-medium shadow-md" onClick={closeMobileMenu}>
              Registrarse
            </Link>
          </div>
