      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-8 md:px-6 lg:px-0">
        <section className="rounded-3xl border border-border bg-surface p-6 shadow-sm md:p-8">
          ...
          <div>
            ...
            {loading ? <div className="rounded-2xl border border-border bg-surface-muted p-6 text-sm text-muted animate-pulse">Cargando órdenes...</div> : orders.length === 0 ? <div className="rounded-2xl border border-border bg-surface-muted p-6 text-sm text-muted">No hay órdenes registradas para esta empresa.</div> : <CompanyOrdersTable orders={orders} onViewDetails={setSelectedOrder} />}
          </div>
        </section>
      </main>
        ...
        {orders.map((order) => {
          ...
          return (
            <div key={order.id} className="grid grid-cols-[1.6fr_0.7fr_0.8fr_0.7fr] items-center border-b border-border px-5 py-4 last:border-b-0">
              ...
              <div>
                <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold capitalize ${ORDER_STATUS_COLOR[order.status?.toLowerCase()] || 'bg-gray-100 text-gray-700'}`}>{ORDER_STATUS_LABEL[order.status?.toLowerCase()] || order.status || 'Pendiente'}</span>
              </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-6 backdrop-blur-sm">
      <div className="relative max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-3xl border border-border bg-surface p-8 shadow-2xl">
        <button type="button" onClick={onClose} className="absolute right-5 top-5 flex h-10 w-10 items-center justify-center rounded-full border border-border bg-surface-muted text-muted transition hover:border-primary/40 hover:text-primary">
          <X size={20} />
        </button>
        ...
        <div className="flex flex-col gap-8 lg:flex-row">
          <div className="w-full space-y-6 lg:w-1/3">
            ...
            <div className="rounded-2xl border border-primary/10 bg-primary/5 p-4 text-center">
              ...
              <p className={`text-lg font-bold uppercase tracking-widest ${ORDER_STATUS_COLOR[order.status?.toLowerCase()] || 'text-gray-500'}`}>{ORDER_STATUS_LABEL[order.status?.toLowerCase()] || order.status}</p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-6">
            <div className="pr-10">
              ...
              <p className="font-mono text-muted">descripcion: {pkg.description ? (pkg.description.length > 80 ? `${pkg.description.substring(0, 80)}...` : pkg.description) : 'Sin descripción'}</p>
            </div>
