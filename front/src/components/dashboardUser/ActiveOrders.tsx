...
import { ORDER_STATUS_LABEL, ORDER_STATUS_COLOR, FINAL_ORDER_STATUSES, type OrderStatus } from '@/constants/orderStatus';

type FinalOrderStatus = (typeof FINAL_ORDER_STATUSES)[number];
...
export default function ActiveOrders({ orders: initialOrders }: ActiveOrdersProps) {
  const activeOrders = (initialOrders || []).filter((o) => {
    ...
    return !FINAL_ORDER_STATUSES.includes(status as FinalOrderStatus);
  });
  ...
  if (selectedOrder) {
    ...
    return (
      <section className="bg-surface rounded-3xl shadow-sm border border-border p-8 animate-in fade-in duration-500">
        <button onClick={() => setSelectedOrder(null)} className="mb-6 text-primary font-semibold flex items-center gap-2 hover:translate-x-1 transition-transform">
          ← Volver al listado
        </button>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-1/3 space-y-6">
            <div className="aspect-square rounded-2xl overflow-hidden border border-border bg-surface-muted">
              <img src={pkg.image || 'https://via.placeholder.com/400?text=Sin+Imagen'} className="w-full h-full object-cover" alt="Producto" />
            </div>
            <div className="bg-primary/5 p-4 rounded-2xl border border-primary/10 text-center">
              ...
              <p className={`text-lg font-bold uppercase tracking-widest ${ORDER_STATUS_COLOR[selectedOrder.status?.toLowerCase()] || 'text-gray-500'}`}>{ORDER_STATUS_LABEL[selectedOrder.status?.toLowerCase()] || selectedOrder.status}</p>
            </div>
          </div>
  ...
  return (
    <section className="bg-surface rounded-3xl shadow-sm border border-border p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="bg-primary/10 text-primary text-sm font-semibold px-4 py-1.5 rounded-full uppercase tracking-wider">{orders.length} activos</span>
      </div>

      <div className="space-y-4">
        {orders.length === 0 ? (
          <div className="bg-surface-muted border border-dashed border-border rounded-2xl p-10 text-center text-muted">No tienes pedidos en camino por ahora.</div>
        ) : (
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="flex items-center gap-4 flex-1">
                  <div className="w-20 h-20 rounded-xl overflow-hidden border border-border bg-surface shrink-0">
                    <img src={order.package?.image || 'https://via.placeholder.com/120'} alt="Producto" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  </div>
                ...
                <div className="flex flex-col items-end gap-2">
                  <span className={`w-fit text-[10px] font-black px-3 py-1 rounded-md uppercase ${ORDER_STATUS_COLOR[order.status?.toLowerCase()] || 'bg-gray-100 text-gray-700'}`}>{ORDER_STATUS_LABEL[order.status?.toLowerCase()] || order.status}</span>
