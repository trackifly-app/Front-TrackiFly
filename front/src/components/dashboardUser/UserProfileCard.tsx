...
export default function UserProfileCard() {
  ...
  const { isEditing, startEditing, form, updateField, reset, save, loading } =
    useUserEditor(profile, userId, (updatedProfile) => {
      if (!userData) return;
  ...
  const handleImageChange = async (
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/profiles/user/${userId}/image`,
        {
          method: "PUT",
          credentials: "include",
          body: formData,
      ...
      let data: {
        ...
        user?: { profile?: { profile_image?: string } };
        message?: string;
      };
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div className="flex items-center gap-4">
          <button
            ...
            className="group relative w-20 h-20 rounded-full overflow-hidden border border-border bg-surface-muted shrink-0 disabled:cursor-not-allowed disabled:opacity-70"
